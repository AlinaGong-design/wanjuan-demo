...
interface WizardData {
  ...
  responseStyle: string; restrictions: string[]; accessScope: string; channels: string[];
}
const initWizardData = (): WizardData => ({
  ...
  accessScope: 'dept', channels: ['embed', 'api'],
});
...
const CreateWizard: React.FC<{ onClose: () => void; initialData?: Partial<WizardData>; isEdit?: boolean }> = ({ onClose, initialData, isEdit }) => {
  ...
  const handleSendTest = () => {
    ...
    setTimeout(() => {
      ...
      const replies = [
        `您好！我是「${name}」，${role}。已收到您的问题，正在基于知识库为您检索相关信息…`,
        `根据您的需求，我将从以下角度为您分析：\n\n1. 初步判断属于${role}的职责范围\n2. 建议结合知识库内容深入处理\n3. 如需人工介入，可随时呼叫`,
      ];
  ...
  const Step0 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', background: 'linear-gradient(135deg, #f5f4ff, #f0f9ff)', borderRadius: 12, border: '1px solid #e8e8f0' }}>
        <div style={{ width: 56, height: 56, borderRadius: 14, flexShrink: 0, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 24, fontWeight: 700 }}>
          {avatarChar}
        </div>
        <div style={{ flex: 1 }}>
          <Input value={data.name} onChange={e => update({ name: e.target.value })} placeholder="输入员工名称，如：法务合规助手" style={{ fontSize: 15, fontWeight: 600, borderRadius: 8 }} bordered />
          <div style={{ fontSize: 11, color: '#aaa', marginTop: 5 }}>头像根据名称自动生成</div>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
        <div>
          <div style={{ fontSize: 13, color: '#555', marginBottom: 6, fontWeight: 500 }}>所属部门 <span style={{ color: '#ff4d4f' }}>*</span></div>
          <Select value={data.dept || undefined} onChange={v => update({ dept: v })} placeholder="选择部门" style={{ width: '100%' }} options={DEPARTMENTS.map(d => ({ label: d, value: d }))} />
        </div>
        <div>
          <div style={{ fontSize: 13, color: '#555', marginBottom: 6, fontWeight: 500 }}>业务域 <span style={{ color: '#ff4d4f' }}>*</span></div>
          <Select value={data.domain || undefined} onChange={v => update({ domain: v })} placeholder="选择业务域" style={{ width: '100%' }} options={DOMAIN_LIST.filter(d => d !== '全部').map(d => ({ label: d, value: d }))} />
        </div>
        <div>
          <div style={{ fontSize: 13, color: '#555', marginBottom: 6, fontWeight: 500 }}>角色定位 <span style={{ color: '#ff4d4f' }}>*</span></div>
          <Input value={data.role} onChange={e => update({ role: e.target.value })} placeholder="如：法务合规顾问" />
        </div>
      </div>
      <div>
        <div style={{ fontSize: 13, color: '#555', marginBottom: 6, fontWeight: 500 }}>功能描述 <span style={{ fontSize: 12, color: '#bbb', fontWeight: 400 }}>（选填）</span></div>
        <Input.TextArea value={data.description} onChange={e => update({ description: e.target.value })} rows={3} placeholder="简要描述该员工的核心能力和适用场景..." style={{ borderRadius: 8, resize: 'none' }} maxLength={200} showCount />
      </div>
    </div>
  );
  ...
  const Step1 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        <SectionTitle title="万卷平台技能" desc="选择该员工可调用的 Skills / 工作流 / MCP" />
        <div style={{ border: '1px solid #f0f0f0', borderRadius: 10, overflow: 'hidden' }}>
      ...
      {/* MCP Server 绑定 */}
      <div>
        <SectionTitle title="MCP Server 绑定" desc="绑定后员工可直接读写企业内部系统数据，实现真正的「数字员工」而非「聊天机器人」" />
        <div style={{ border: '1px solid #f0f0f0', borderRadius: 10, overflow: 'hidden' }}>
        ...
        <div style={{ marginTop: 8, padding: '8px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, fontSize: 11, color: '#16a34a' }}>
          💡 MCP 绑定后，员工可直接查询数据库、写入飞书、创建 GitLab PR，无需用户手动粘贴数据
        </div>
      </div>
    </div>
  );
  ...
  const Step2 = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {RESPONSE_STYLES.map(rs => (
            <div key={rs.id} onClick={() => update({ responseStyle: rs.id })} style={{ flex: 1, padding: 12, borderRadius: 10, cursor: 'pointer', textAlign: 'center', border: data.responseStyle === rs.id ? '1.5px solid #6366F1' : '1px solid #e8e8e8', background: data.responseStyle === rs.id ? '#f5f4ff' : '#fafafa', transition: 'all 0.15s' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[{ value: 'private', icon: '🔒', label: '仅创建者', desc: '私有，仅自己可用' }, { value: 'dept', icon: '👥', label: '部门内', desc: '部门成员可访问' }, { value: 'company', icon: '🌐', label: '全公司', desc: '全员可见可用' }].map(opt => (
            <div key={opt.value} onClick={() => update({ accessScope: opt.value })} style={{ flex: 1, padding: 12, borderRadius: 10, cursor: 'pointer', textAlign: 'center', border: data.accessScope === opt.value ? '1.5px solid #6366F1' : '1px solid #e8e8e8', background: data.accessScope === opt.value ? '#f5f4ff' : '#fafafa', transition: 'all 0.15s' }}>
  ...
  const Step3 = () => {
    ...
    const configItems = [
      { label: '员工身份', ok: !!data.name && !!data.dept && !!data.role, text: data.name ? `${data.name} · ${data.dept} · ${data.domain || '未选域'}` : '未配置' },
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', border: '1px solid #e0deff', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid #e0deff', display: 'flex', alignItems: 'center', gap: 8, background: '#fff' }}>
            <div style={{ width: 26, height: 26, borderRadius: 7, flexShrink: 0, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700 }}>{avatarChar}</div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', minHeight: 200, maxHeight: 280, background: '#f8f7ff', display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              <div style={{ width: 26, height: 26, borderRadius: 7, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{avatarChar}</div>
              <div style={{ background: '#fff', borderRadius: '0 10px 10px 10px', padding: '8px 12px', fontSize: 12, color: '#333', maxWidth: '80%', border: '1px solid #e8e8e8', lineHeight: 1.6 }}>
                你好！我是{data.name || '数字员工'}，{data.role || '你的智能助理'}，请描述您的需求。
              </div>
            </div>
            {testMessages.map((msg, idx) => (
              <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
                {msg.role === 'ai' && <div style={{ width: 26, height: 26, borderRadius: 7, background: avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{avatarChar}</div>}
                <div style={{ background: msg.role === 'user' ? '#6366F1' : '#fff', borderRadius: msg.role === 'user' ? '10px 0 10px 10px' : '0 10px 10px 10px', padding: '8px 12px', fontSize: 12, color: msg.role === 'user' ? '#fff' : '#333', maxWidth: '80%', border: msg.role === 'user' ? 'none' : '1px solid #e8e8e8', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <div style={{ padding: '10px 12px', borderTop: '1px solid #e0deff', background: '#fff', display: 'flex', gap: 8 }}>
            <Input value={testInput} onChange={e => setTestInput(e.target.value)} placeholder={`向 ${data.name || '数字员工'} 发送测试消息...`} style={{ borderRadius: 8, flex: 1, fontSize: 12 }} onPressEnter={handleSendTest} />
            <Button type="primary" icon={<SendOutlined />} disabled={!testInput.trim() || testLoading} style={{ background: '#6366F1', borderColor: '#6366F1', borderRadius: 8 }} onClick={handleSendTest} />
          </div>
        </div>
        <div style={{ width: 210, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ border: '1px solid #f0f0f0', borderRadius: 10, overflow: 'hidden' }}>
            {configItems.map((item, idx) => (
              <div key={item.label} style={{ padding: '8px 12px', borderBottom: idx < configItems.length - 1 ? '1px solid #f5f5f5' : 'none', background: item.ok ? '#fff' : '#fffbeb' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          ...
          {allOk
            ? <div style={{ padding: '8px 12px', background: '#f6ffed', border: '1px solid #b7eb8f', borderRadius: 8, fontSize: 11, color: '#52c41a', display: 'flex', gap: 5 }}><CheckCircleOutlined /><span>配置完整，可以发布</span></div>
            : <div style={{ padding: '8px 12px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, fontSize: 11, color: '#d97706' }}>⚠️ 部分配置待完善，可先保存草稿</div>
          }
        </div>
      </div>
    );
  ...
  return (
    <div>
      <Steps current={step} size="small" style={{ marginBottom: 24 }} items={STEPS.map(s => ({ title: s.title, description: s.description }))} />
      <div style={{ minHeight: 340, maxHeight: 520, overflowY: 'auto', paddingRight: 4 }}>{STEP_CONTENT[step]}</div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20, paddingTop: 14, borderTop: '1px solid #f0f0f0' }}>
        <Button onClick={step === 0 ? onClose : () => setStep(s => s - 1)}>{step === 0 ? '取消' : '上一步'}</Button>
        <div style={{ display: 'flex', gap: 10 }}>
          {step === STEPS.length - 1 ? (
            <>
              <Button icon={<CloudUploadOutlined />} onClick={() => { onClose(); message.success('草稿已保存，可在员工库继续编辑'); }}>保存草稿</Button>
              <Button type="primary" icon={<CheckCircleOutlined />} style={{ background: '#6366F1', borderColor: '#6366F1' }} onClick={() => { onClose(); message.success(`数字员工「${data.name}」已发布上线`); }}>发布上线</Button>
            </>
          ) : (
            <>
              {step > 0 && <Button onClick={() => { onClose(); message.success('草稿已保存'); }}>保存草稿</Button>}
              <Button type="primary" disabled={!canNext()} style={{ background: '#6366F1', borderColor: '#6366F1' }} onClick={() => setStep(s => s + 1)}>下一步</Button>
            </>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => setCreateModalOpen(true)} style={{ background: '#6366F1', borderColor: '#6366F1', borderRadius: 8, fontWeight: 600 }}>创建数字员工</Button>
      </div>
      ...
      {/* 创建 Modal */}
      <Modal title={<span style={{ fontSize: 15, fontWeight: 700 }}>创建数字员工</span>} open={createModalOpen} onCancel={() => setCreateModalOpen(false)} footer={null} width={760} destroyOnClose styles={{ body: { maxHeight: '80vh', overflowY: 'auto' } }}>
        <CreateWizard onClose={() => setCreateModalOpen(false)} />
      </Modal>

      {/* 编辑 Modal */}
      <Modal title={<span style={{ fontSize: 15, fontWeight: 700 }}>编辑数字员工</span>} open={editModalOpen} onCancel={() => { setEditModalOpen(false); setEditEmployee(null); }} footer={null} width={760} destroyOnClose styles={{ body: { maxHeight: '80vh', overflowY: 'auto' } }}>
        <CreateWizard onClose={() => { setEditModalOpen(false); setEditEmployee(null); }} initialData={{ name: editEmployee?.name ?? '', dept: editEmployee?.dept ?? '', description: editEmployee?.description ?? '', domain: editEmployee?.domain ?? '' }} isEdit />
      </Modal>
